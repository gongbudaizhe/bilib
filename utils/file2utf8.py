...
TODO:
    ...
    DONE! for large files, it can consume a lot of memory(for 2.5G file,
            you need a machine with memory larger than 6G),
            I shall figure out a way to solve this.
...
def get_available_encodings():
    ...
    found = set(name for imp, name, ispkg in
        ...
        for name in filenames:
            ...
            with open(old_file) as f_in:
                with open(new_file, "w") as f_out:
                    f_out.write(convert_to_utf8(f_in.read(),
                                                autodetect,
                                                first_try_cp))
...
def find_encoding(string, autodetect=True, first_try_cp=None):
    ...
    string_partial = string[:8000]  # we don't need all of the string to find
                                    # the encoding
...
if __name__ == '__main__':
    ...
    if encoding:
        while True:
            ...
            if not status:
                sys.stderr.write("number of errors: " + str(errors) + "\n")
                break
    else:
